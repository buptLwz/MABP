...
class G_REFER:
    ...
    def createIndex(self):
        ...
        # 1)  Refs: 	 	{ref_id: ref}
        # 2)  Anns: 	 	{ann_id: ann} #是一个int:int类型的字典，其中-1对应None，代表无目标的refer
        # 3)  Imgs:		 	{image_id: image} #每个图片image_id可能包含多个实例ref_id它的mask和类别唯一（ann_id和cat_id)，每个实例又对应多个句子sent_id，
        # 4)  Cats: 	 	{category_id: category_name}
        # 5)  Sents:     	{sent_id: sent} #句子的id，每个图片中的每个实例可能对应多个句子，所以在ref中会在每个实例的信息开头声明该实例包含的所有句子id，然后再依次给出每个句子的id和它的token
        ...
        for ann in self.data['annotations']:
            Anns[ann['id']] = ann
            imgToAnns[ann['image_id']] = imgToAnns.get(ann['image_id'], []) + [ann] #通过当前ann索引到它所在的图片，然后再把该图片image_id下已有的Ann都取出来，默认值为[]，然后和当前ann拼接起来
