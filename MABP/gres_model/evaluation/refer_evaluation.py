...
class ReferEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        for input, output in zip(inputs, outputs):
            ...
            # output mask
            output_mask = (output["ref_seg"]>0.5).to(self._cpu_device)
            ...
            # output NT label
            #output_nt = [(output["nt_label"].argmax(dim=0)).bool().to(self._cpu_device) for i in output["nt_label"]]
            # output_nt = [(i.argmax(dim=0)).bool().to(self._cpu_device) for i in output["nt_label"]]
            # output_nt = any(output_nt)
            if self.so:
                output_nt = torch.tensor(0).bool().to(self._cpu_device)
            else:
                output_nt = [(i.argmax(dim=0)).bool().to(self._cpu_device) for i in output["nt_label"]]
                output_nt = any(output_nt)
