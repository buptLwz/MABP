...
class GRES(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        text_encoder = BertModel.from_pretrained('/root/autodl-tmp/bert-base-uncased')
        ...
        A_weight = cfg.MODEL.MASK_FORMER.A_WEIGHT
        B_weight = cfg.MODEL.MASK_FORMER.B_WEIGHT
        C_weight = cfg.MODEL.MASK_FORMER.C_WEIGHT
        D_weight = cfg.MODEL.MASK_FORMER.D_WEIGHT
        E_weight = cfg.MODEL.MASK_FORMER.E_WEIGHT



        weight_dict = {"A": A_weight, "B": B_weight, "C": C_weight, "D": D_weight,"E": E_weight}
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            #masks = outputs['all_masks']
            #gathers = outputs['all_gathers']
            #last_mask = outputs['last_mask']
            mask_pred_results = outputs["pred_masks"][-1].unsqueeze(1)
            # upsample masks
            ...
            nt_pred_results = outputs["nt_label"]#[-1]
            ...
            for mask_pred_result, nt_pred_result, input_per_image, image_size in zip(
                ...
                #gr, ma = retry_if_cuda_oom(self.refer_inference)(gathers, masks)
                processed_results[-1]["ref_seg"] = r
                processed_results[-1]["nt_label"] = nt
                #processed_results[-1]["last_mask"] = last_mask

                #processed_results[-1]["gathers"] = gr
                #processed_results[-1]["masks"] = ma
