...
class MultiScaleMaskedReferringDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, lang_feat, lang_mask, lang_pwam):
        ...
        for i in range(self.num_layers):
            ...
            lang_layer_embed = (lang_v*lang_layer_weight.unsqueeze(1)).permute(2,0,1) #B,256,20 -> 20,B,256
            ...
            if (i + 1) % self.num_feature_levels == 0:
                nt_lang_emb = fusion_output[output.shape[0]:,:,:].permute(1,2,0) #b,256,20


                #nt_emb = torch.einsum("lbc,qbc->")
                outputs_mask_gathered, nt_label = \
                self.forward_prediction_heads(output, mask_features, src, size_list, i, False, nt_lang_emb)
                predictions_mask.append(outputs_mask_gathered)
                predictions_nt_label.append(nt_label)
                ...
                if level_index==2:
                    ...
                    outputs_mask_gathered, nt_label= \
                        self.forward_prediction_heads(output_final, mask_features, src, size_list, i+1, True,nt_lang_emb)
                    predictions_mask.append(outputs_mask_gathered)
                    predictions_nt_label.append(nt_label)
    ...
    def forward_prediction_heads(self, output, mask_features, src, srcsize, i, fpn, nt_lang_emb):
        ...
        nt_lang_weight = self.nt_lang_emb_proj[level_index](nt_lang_emb) # b,256,20->b,256,1
        
        nt_lang_weight = torch.einsum('bca,qbc->baq',nt_lang_weight,output) #b,1,q
