...
class MultiScaleMaskedReferringDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, lang_feat, lang_mask, lang_pwam):
        ...
        attn_mask = self.attn_mask_generate(output, src, size_list[0], -1)
        ...
        for i in range(self.num_layers):
            ...
            if i > self.num_layers - self.num_feature_levels-1:
                outputs_mask_gathered, nt_label = \
                self.forward_prediction_heads(output, mask_features, src, size_list[(i)%self.num_feature_levels], i, False)
                predictions_mask.append(outputs_mask_gathered)
                predictions_nt_label.append(nt_label)
...
###################
                ...
                outputs_mask_gathered, nt_label = \
                    self.forward_prediction_heads(output_final, mask_features, src, size_list[(i+1)%self.num_feature_levels], i+1, True)
                predictions_mask.append(outputs_mask_gathered)
                predictions_nt_label.append(nt_label)
            #predictions_minimap.append(outputs_class)
