...
class MultiScaleMaskedReferringDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        for i in range(self.num_feature_levels+1):    
            ...
            self.nt_lang_emb_proj.append(MLP(256,256,256,2))
        ...
        self.lang_query_embed = nn.Embedding(1, hidden_dim)
    ...
    def forward(self, x, mask_features, lang_feat, lang_mask, lang_pwam):
        ...
        for i in range(self.num_layers):
            ...
            if i > 0:
                ...
                cur_attn_mask = torch.cat([attn_mask,torch.zeros((attn_mask.shape[0],attn_mask.shape[1],1),device=output.device,dtype=bool)],dim=-1)
            else:
                attn_mask = None
                cur_attn_mask = None
            ...
            #lang_v = self.layer_embed[i](lang_v)
           ...
           # attention: cross-attention first
            ...
            if (i + 1) % self.num_feature_levels == 0:
                nt_lang = lang_v#.permute(1,2,0) #1,b,256->b,256,20
    ...
    def forward_prediction_heads(self, output, mask_features, src, srcsize, i, fpn, nt_lang):
        ...
        nt_lang_emb = self.nt_lang_emb_proj[level_index](nt_lang) # 1,b,256
        
        nt_lang_emb = torch.einsum('abc,bqc->baq',nt_lang_emb,nt_emb) #b,1,q
        #print(nt_lang_weight.shape)
        #nt_label = torch.sigmoid(nt_lang_weight)*nt_src_emb
        #print(nt_label.shape)
        nt_label = torch.cat([nt_lang_emb,nt_src_emb],dim=-1) #b,1,2q
