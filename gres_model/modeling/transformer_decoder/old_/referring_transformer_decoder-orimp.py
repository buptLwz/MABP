...
def make_coord(shape,decive, ranges=None, flatten=True):
    ...
    for i, n in enumerate(shape):
        if ranges is None:
            ...
            v0, v1 = ranges[i]
        r = (v1 - v0) / (2 * n)
        seq = v0 + r + (2 * r) * torch.arange(n,device=decive).float()
...
def ifa_feat(res, size, stride=1, local=False):
    ...
    h , w = size
    coords = (make_coord((h,w),res.device).flip(-1) + 1) / 2
    #coords = (make_coord((h,w)).flip(-1) + 1) / 2
    coords = coords.unsqueeze(0).expand(bs, *coords.shape)
    coords = (coords*2-1).flip(-1)

    feat_coords = make_coord((hh,ww),res.device, flatten=False).permute(2, 0, 1) .unsqueeze(0).expand(res.shape[0], 2, *(hh,ww))
...
def ifa_upsample(x,shape,pos_fn):
    rel_coord_list, q_feat_list, area_list = ifa_feat(x,shape,  local=True, stride=1)
    total_area = torch.stack(area_list).sum(dim=0)
    context_list = []
    for rel_coord, q_feat, area in zip(rel_coord_list, q_feat_list, area_list):
        rel_coord = pos_fn(rel_coord)
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dropout=0.0,
                 activation="relu", normalize_before=False,view=False):
        super().__init__()
    ...
    def forward_post(self, tgt, memory,
        ...
        # if self.view:
        #     import json
        #     save_output = weight.detach().cpu().numpy()
        #     #save_output = np.around(save_output,2)
        #     save_output = save_output.tolist()
        #     save_output = json.dumps(save_output)
        #     with open('./attn.json',"w",encoding='utf-8') as f:
        #         f.write(save_output+',\n')
        tgt = tgt + self.dropout(tgt2)
        tgt = self.norm(tgt)
...
class MultiScaleMaskedReferringDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        #self.layer_embed_proj = Linear(768,256)
        ...
        for i in range(self.num_feature_levels+1):    
            ...
            self.nt_embed.append(MLP(2*qq, 2*qq, 2, 2)) # nn.Linear(hidden_dim, num_classes + 1)

            self.mask_embed.append(MLP(hidden_dim, hidden_dim, mask_dim, 3))
            self.decoder_norm.append( nn.LayerNorm(hidden_dim))
            self.nt_lang_emb_proj.append(MLP(20,20,1,2))
        ...
        # self.ifa_pos =  nn.ModuleList()
        # self.ifa_mlp_query = nn.ModuleList()
        # self.ifa_mlp_query_emb = nn.ModuleList()

        # for _ in range(self.num_feature_levels-1):  
        #     self.ifa_pos.append(SpatialEncoding(2, 24, require_grad=True))
        #     self.ifa_mlp_query.append(MLP(hidden_dim+26,hidden_dim,hidden_dim,2))
        #     self.ifa_mlp_query_emb.append(MLP(hidden_dim+26,hidden_dim,hidden_dim,2))
        ...
        #self.lang_layer_embed = nn.Embedding(self.num_layers, 20)
    ...
    def forward(self, x, mask_features, lang_feat, lang_mask, lang_pwam):
        ...
        #lang_layer_embed = self.layer_embed_proj(lang_feat).permute(0,2,1)
        ...
        for i in range(self.num_layers):
            ...
            #lang_layer_weight = torch.sigmoid(self.lang_layer_embed.weight[i]).unsqueeze(0).repeat(bs,1)
            #lang_layer = (lang_v*lang_layer_weight.unsqueeze(1)).permute(2,0,1) #B,256,20 -> 20,B,256
            #lang_attn_mask[torch.where(lang_attn_mask.sum(-1) == lang_attn_mask.shape[-1])] = False
            #src HW,N,C
            #cur_src = src[level_index]
           ...
           # attention: cross-attention first
            ...
            #lang_layer_embed = self.layer_embed[i](lang_layer_embed)
            ...
            if (i + 1) % self.num_feature_levels == 0:
                ...
                #nt_emb = torch.einsum("lbc,qbc->")
                outputs_mask_gathered, nt_label = \
                self.forward_prediction_heads(output, mask_features, src, size_list, i, False, nt_lang)
                predictions_mask.append(outputs_mask_gathered)
                predictions_nt_label.append(nt_label)
                ...
                if level_index==0 or (level_index==1):
                    ...
                    #output = ifa_upsample(output,(sqrtq*2,sqrtq*2),self.ifa_pos[level_index])
                    #print(output.shape)
                    #output = self.ifa_mlp_query[level_index](output).permute(1,0,2)#.permute(0,2,1).reshape(bs,channels,sqrtq*2,sqrtq*2)

                    #query_embed = ifa_upsample(query_embed,(sqrtq*2,sqrtq*2),self.ifa_pos[level_index])
                    #query_embed = self.ifa_mlp_query_emb[level_index](query_embed).permute(1,0,2)#.permute(0,2,1).reshape(bs,channels,sqrtq*2,sqrtq*2)
                ...
                if level_index==2:
                    ...
                    # if True:
                    #     import json
                    #     save_output = out[-1].detach().cpu().numpy()
                    #     #save_output = np.around(save_output,2)
                    #     save_output = save_output.tolist()
                    #     save_output = json.dumps(save_output)
                    ...
                    outputs_mask_gathered, nt_label= \
                        self.forward_prediction_heads(output_final, mask_features, src, size_list, i+1, True,nt_lang)
                    predictions_mask.append(outputs_mask_gathered)
                    predictions_nt_label.append(nt_label)
    ...
    def forward_prediction_heads(self, output, mask_features, src, srcsize, i, fpn, nt_lang):
        ...
        if fpn == False:
            ...
            #pooled_src = F.adaptive_max_pool2d(src_sw.flatten(0,1).permute(0,3,1,2),(1,1)).squeeze(-1).squeeze(-1).reshape_as(mask_embed)
            ...
            # if True:
            #     import json
            #     save_output = mask_embed.detach().cpu().numpy()
            #     #save_output = np.around(save_output,2)
            #     save_output = save_output.tolist()
            #     save_output = json.dumps(save_output)
            #     with open('./queries2.json',"w",encoding='utf-8') as f:
            #         f.write(save_output+',\n')
            ...
            #pooled_src = F.adaptive_max_pool2d(mask_sw.flatten(0,1).permute(0,3,1,2),(1,1)).squeeze(-1).squeeze(-1).reshape_as(mask_embed) #b,q,c
            ...
            if True:
                ...
                import numpy as np
                import json
                save_output = np.uint8(outputs_mask_curlvl.sigmoid().squeeze().detach().cpu().numpy()>0.5)
                
                #save_output = np.around(save_output,2)
                #save_output = save_output.tolist()
                mask = Image.fromarray(save_output*255)
                mask.save('./mask-curlvl-all_ori2.png')
                #save_output = json.dumps(save_output)
                #with open('./masklvl-decoder-ori.json',"w",encoding='utf-8') as f:
                    #f.write(save_output+',\n')
                assert 1==2
        ...
        #print(nt_lang_weight.shape)
        #nt_label = torch.sigmoid(nt_lang_weight)*nt_src_emb
        #print(nt_label.shape)
        #nt_label = nt_lang_emb-nt_src_emb
