...
class MultiScaleMaskedReferringDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, lang_feat, lang_mask, lang_pwam):
        ...
        for i in range(self.num_layers):
            ...
            lang_layer_embed = self.layer_embed[i](lang_feat).permute(1,0,2) #B,20,768 -> 20,B,256
            ...
            lang_layer_embed = fusion_output[output.shape[0]:,:,:]#.permute(0,2,1)
            ...
            if (i + 1) % self.num_feature_levels == 0:
                ...
                self.forward_prediction_heads((output,query_embed), (lang_layer_embed,lang_query_embed), mask_features, src, size_list, i, False)
                predictions_mask.append(outputs_mask_gathered)
                predictions_nt_label.append(nt_label)
                ...
                if level_index==2:
                    for idx, x in enumerate(output_fpn):
                        ...
                        y = cur_fpn + F.interpolate(out[-1], size=cur_fpn.shape[-2:], mode="nearest")
                        y = output_conv(y)
                        #y = F.leaky_relu(y)
                    ...
                    outputs_mask_gathered, nt_label= \
                        self.forward_prediction_heads((output_final,query_embed), (lang_layer_embed,lang_query_embed), mask_features, src, size_list, i+1, True)
                    predictions_mask.append(outputs_mask_gathered)
                    predictions_nt_label.append(nt_label)
        ...
        predictions_nt_label = torch.cat(predictions_nt_label,dim=0).squeeze(-1)
        predictions_nt_label = self.nto_proj(predictions_nt_label.permute(1,0))
        out = {
    ...
    def forward_prediction_heads(self, output, lang_layer_embed, mask_features, src, srcsize, i, fpn):
        ...
        nt_label = self.nt_embed[level_index](nt_emb)
