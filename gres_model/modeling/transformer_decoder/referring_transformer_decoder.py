...
class MultiScaleMaskedReferringDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        for i in range(self.num_feature_levels+1):  
            ...
            qq = (self.num_queries*(4**(min(i,self.num_feature_levels-1)))) #number of queries
    ...
    def forward(self, f0,f1,f2, mask_features, lang_feat, lang_mask):
        ...
        for i in range(self.num_layers):
            ...
            if i > 0:
                ...
                cur_attn_mask = torch.cat([attn_mask,torch.zeros((attn_mask.shape[0],attn_mask.shape[1],self.text_len),device=output.device,dtype=bool)],dim=-1)
            else:
                attn_mask = None
                cur_attn_mask = None
    ...
    def attn_mask_generate(self, output, src, srcsize, level_index):
        '''Generate the attention mask only forward the main branch (detached)'''
