...
'''same as ReLA, but we provide deterministic parameters to assist with the future application of torch.compile.'''
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,idx=0,layer=0):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        # deterministic parameters for input size = 480*480
        list=[[120,120],[60,60],[30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30],[15,15]]
        self.H = list[layer][idx]
        self.W = list[layer][idx]
        self.idx = idx
...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww,lang= layer(x, Wh, Ww, l, l_mask)
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                norm_layer=norm_layer,idx=i,layer=layer)
        ...
        # deterministic parameters for input size=480*480
        self.HWp=np.array([120,60,36,24])

        self.img_mask=[torch.zeros((1,i,i,1),device='cuda') for i in self.HWp]
        for i in range(len(self.img_mask)):
            self.img_mask[i][:, 0:-self.window_size, 0:-self.window_size, :] = 0
            self.img_mask[i][:, 0:-self.window_size, -self.window_size: -self.shift_size, :] = 1
            self.img_mask[i][:, 0:-self.window_size, -self.shift_size:None, :] = 2
    
            self.img_mask[i][:, -self.window_size: -self.shift_size,0:-self.window_size, :] = 3
            self.img_mask[i][:, -self.window_size: -self.shift_size,-self.window_size: -self.shift_size, :] = 4
            self.img_mask[i][:, -self.window_size: -self.shift_size,-self.shift_size:None, :] = 5
    
            self.img_mask[i][:, -self.shift_size:None, 0:-self.window_size, :] = 6
            self.img_mask[i][:, -self.shift_size:None, -self.window_size: -self.shift_size:, :] = 7
            self.img_mask[i][:, -self.shift_size:None, -self.shift_size:None, :] = 8
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        # directly use deterministic parameter
        # if H==120:
        #     img_mask = self.img_mask[0]  # 1 Hp Wp 1
        # elif H==60:
        #     img_mask = self.img_mask[1]  # 1 Hp Wp 1
        # elif H==30:
        #     img_mask = self.img_mask[2]  # 1 Hp Wp 1
        # elif H==15:
        #     img_mask = self.img_mask[3]  # 1 Hp Wp 1
        ...
        # PWAM fusion
        x_residual,lang = self.fusion(x, l, l_mask)
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x_residual, H, W, x_down, Wh, Ww,lang
        else:
            return x_residual, H, W, None, H, W,lang
